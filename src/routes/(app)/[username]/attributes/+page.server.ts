...
import type { Attribute, DB } from '$lib/server/db/kysely-codegen.js'
...
export const load: PageServerLoad = (async ({ parent, url }) => {
	...
	const query = db.selectFrom('attributes').limit(limit).selectAll().where('user', '=', profile.id)
	const withSort = setSort(query, url)
	const withCursor = setCursor(withSort, url)

	const attributes = await withCursor.execute()
	return { attributes }
}) satisfies PageServerLoad

function setSort<T>(query: SelectQueryBuilder<DB, 'attributes', T>, url: URL) {
...
} as const satisfies Record<Sortable, keyof Attribute>
